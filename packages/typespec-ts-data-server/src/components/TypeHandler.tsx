...
export function TypeHandler(props: TypeHandlerProps) {
  ...
  return <>
    ...
    <InterfaceDeclaration name={props.type.name + "Handler"} refkey={refkey(props.type, "handler-interface")} export>
      list(): <HandlerReturnType>{refkey(props.type, "ts-read")}[]</HandlerReturnType>;
      create(data: {refkey(props.type, "ts-create")}): <HandlerReturnType>{refkey(props.type, "ts-read")}</HandlerReturnType>;
      read(id: <TypeExpression type={keyProp} />): <HandlerReturnType>{refkey(props.type, "ts-read")}</HandlerReturnType>;
      update(id: <TypeExpression type={keyProp} />, data: {refkey(props.type, "ts-update")}): <HandlerReturnType>{refkey(props.type, "ts-read")}</HandlerReturnType>;
      delete(id: <TypeExpression type={keyProp} />): <HandlerReturnType>void</HandlerReturnType>
...
function HandlerReturnType(props: HandlerReturnTypeProps) {
  return <>
    Promise{"<"}{refkey("http-envelope")}{"<"}{props.children}{">"}{">"}
  </>;
}
